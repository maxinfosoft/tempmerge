...
/*
    ...
    //https://kb.epam.com/display/PGUSDAV/BCP+Procedure#BCPProcedure-CheckpointJob
    parameters {
        ...
        string(name: 'snapshot_id_old', defaultValue: '20200331', description: 'Previous snapshot used for the validation step. Comparing ETLs.')
        ...
        string(name: 'fpi_datasets_folder', defaultValue: 'davinci-upload-uk', description: 'Folder name where source dataset for fallback run is uploaded.')
        string(name: 'FALLBACK_TIMEOUT_IN_SECONDS', defaultValue: '3600', description: '')
    }
*/
    ...
    stages {
        ...
        stage ('Parallel Stage') {
            ...
            parallel {
                ...
                stage ('Wait to trigger Fallback'){

                    steps {

                        script {
                            def time = params.FALLBACK_TIMEOUT_IN_SECONDS.toInteger()
                            def elapsed = 0
                            echo "Waiting ${FALLBACK_TIMEOUT_IN_SECONDS} seconds for 'Wait for Files' Stage result"
                            while(elapsed <= time && StageSucceed ==false) {

                                echo "Wait to trigger Fallback: Sleeping for 120 seconds..."
                                sleep(120)
                                elapsed = elapsed + 120
                            }
                            // echo "Waiting ${FALLBACK_TIMEOUT_IN_SECONDS} minutes for wait second stage result"
                            // sleep time.toInteger() // seconds
                        }

                        script {
                            if (StageSucceed) {
                                sh "exit 0" // the main stage is successful, we finish this stage.
                            } else {
                                // The main stage ended with an error or was not executed via FALLBACK_TIMEOUT_IN_SECONDS

                                script {
                                    fallback_job_url_params = get_URL_encoded_params([
                                            ['branch_name', params.branch_name],
                                            ['location', params.location],
                                            ['etl_params', params.etl_params],
                                            ['etl_version', params.etl_version],
                                            ...
                                            ['blob_source_container_name', params.blob_source_container_name],
                                            ['datasets_folder', params.datasets_folder],
                                            ['da_gen_params', params.da_gen_params],
                                            ['snapshot_id_old', params.snapshot_id_old],
                                            ['etl_version_old', params.etl_version_old],
                                            ['env', params.env],
                                            ['fpi_datasets_folder', params.fpi_datasets_folder]
                                    ])
                                }

                                echo "fallback job URL params: ${fallback_job_url_params}"
                                ...
                                withCredentials([usernamePassword(credentialsId: "${api_token_shared_id_w_token}", usernameVariable: 'API_USER', passwordVariable: 'API_TOKEN')]) {
                                    sh("curl --insecure -X POST \"${host_prod}/job/Fallback-UK/buildWithParameters?${fallback_job_url_params}\" --user ${API_USER}:${API_TOKEN}")
                                }
                            }
                        }
                    }
        ...
        stage ('Pre-ETL') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    ...
                    sh("sudo azcopy copy  '${storage_account}/${params.blob_source_container_name}/${params.datasets_folder}/${snapshot_id}/${SAS_KEY}' 'data/input/DaVinciProjectOneDrive/' --recursive=true")
                    ...
                    script {
                        try{
                            sh(". /opt/miniconda3/etc/profile.d/conda.sh && ./env_update.sh && conda activate davinci && conda env list && python ./dav.py validation pre-etl -s ${snapshot_id} -d ${snapshot_id} -sp ${params.snapshot_id_old} --source-key '${BLOB_KEY}' --account ${blob_account_name} --container ${params.blob_source_container_name} -f ${params.datasets_folder} -c ${params.location} -l ${params.logs_path} -p \"data/input/DaVinciProjectOneDrive/\"")
                        } finally {
                            // sh("sudo azcopy --quiet  --dest-key \"${BLOB_KEY}\" --destination \"${storage_account}/${target_folder}/validation_logs/${snapshot_id}/${params.location}/pre-etl\" --source \"${params.logs_path}\" --recursive")

                            sh("sudo azcopy copy  '${params.logs_path}' '${storage_account}/${target_folder}/validation_logs/${snapshot_id}/${params.location}/pre-etl${SAS_KEY}' --recursive=true")
                        }
                    }
                }
            }
        ...
        stage ('ETL') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    ...
                    sh("sudo mkdir -p  'data/input/DaVinciProjectOneDrive/Supplemental inputs/' ")
                    sh("sudo mkdir -p  'data/input/DaVinciProjectOneDrive/' ")
                    ...
                    sh("sudo azcopy copy   'data/output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet' '${storage_account}/${target_folder}/etl_output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet${SAS_KEY}'  --recursive=true")
                }
            }
        }
        ...
        stage ('Post-ETL') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    sh(". /opt/miniconda3/etc/profile.d/conda.sh && ./env_update.sh && conda activate davinci && conda env list && python ./dav.py validation prod-pre-etl --source-key '${BLOB_KEY}' --account ${blob_account_name} --container ${params.blob_source_container_name} -f ${params.datasets_folder} -c ${params.location} --update-marker-file ${snapshot_id}")
                    // sh("/usr/bin/keyctl new_session")
                    ...
                    sh("sudo azcopy copy  '${storage_account}/${target_folder}/etl_output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet${SAS_KEY}'  'data/output/' --recursive=true")
                    ...
                    script {
                        try {
                            sh(". /opt/miniconda3/etc/profile.d/conda.sh && ./env_update.sh && conda activate davinci && conda env list && python ./dav.py validation post-etl -c ${params.location} -v ${params.etl_version} -s \"data/output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet\" -p \"data/output/etl_output__${etl_version_old}__${params.location}__${params.snapshot_id_old}.parquet\" --source-key '${BLOB_KEY}' --account ${blob_account_name} --container ${params.blob_source_container_name} -l ${params.logs_path}")

                        } finally {
                            sh("sudo azcopy copy '${params.logs_path}'  '${storage_account}/${target_folder}/validation_logs/${snapshot_id}/${params.location}/post-etl${SAS_KEY}'   --recursive=true")
                            sh("sudo azcopy copy  '${storage_account}/${target_folder}/etl_output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet${SAS_KEY}'  'data/output/' --recursive=true")
                            sh("sudo azcopy copy  'data/output/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet' '${storage_account}/${target_folder}/snapshots/${params.location}/${params.etl_version}/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet${SAS_KEY}'   --recursive=true ")
                        }
                    }
                }
            }
        ...
        stage ('ML') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    ...
                    sh("sudo azcopy copy   'data/experiments/${params.ml_directory}/*' '${storage_account}/${target_folder}/future_predictions/${params.location}/${snapshot_id}${SAS_KEY}'  --recursive=true")
                }
            }
        }
        ...
        stage ('ML extra artifacts') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    sh("sudo azcopy copy \"${storage_account}/${target_folder}/future_predictions/${params.location}/${snapshot_id}/all/all/predictions/*${SAS_KEY}\" \"data/post_ml/predictions\" --recursive=true")
                    sh("sudo azcopy copy \"${storage_account}/${target_folder}/snapshots/${params.location}/${params.etl_version}/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet${SAS_KEY}\" \"data/post_ml/etl/etl_output__${params.etl_version}__${params.location}__${snapshot_id}.parquet\" --recursive=true")
                    sh("sudo azcopy copy \"${storage_account}/${params.blob_source_container_name}/${params.datasets_folder}/${snapshot_id}/pea_event_files/${params.location.toLowerCase()}/*${SAS_KEY}\" \"data/post_ml/pea\" --recursive=true")
                    ...
                    script {
                        try {
                            ...
                            sh("sudo azcopy copy \"data/post_ml/output/*\" \"${storage_account}/${target_folder}/future_predictions/${params.location}/${snapshot_id}/all/all/predictions${SAS_KEY}\" --recursive=true")
                        }
                    }
                }
            }
        ...
        stage ('Post-ML') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    ...
                    sh("sudo azcopy copy  '${storage_account}/${target_folder}/future_predictions/${params.location}/${snapshot_id}/all${SAS_KEY}'   'data/lake/predictions/' --recursive=true")
                    ...
                    script {
                        try {
                            ...
                            sh("sudo azcopy copy  '${params.logs_path}' '${storage_account}/${target_folder}/validation_logs/${snapshot_id}/${params.location}/post-ml${SAS_KEY}'   --recursive=true")
                        }
                    }
                }
            }
        ...
        stage ('DA Generation') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    ...
                    script {
                        if (location == 'UK') {
                            sh("sudo mkdir -p  'data/lake/predictions/' ")
                            sh("sudo azcopy copy  '${storage_account}/${target_folder}/future_predictions/UK/${snapshot_id}/all/all/predictions/PEA_vs_DaVinci_Forecast_per_retailer_promo_barcode.xlsx${SAS_KEY}'  'data/lake/predictions' --recursive=true")
                        }
                    }
                    sh("sudo mkdir -p  'data/input/DaVinciProjectOneDrive/' ")
                    ...
                    sh("sudo azcopy copy  '${storage_account}/${params.blob_source_container_name}/${params.datasets_folder}/${params.snapshot_id_old}${SAS_KEY}'  'data/input/DaVinciProjectOneDrive/' --recursive=true")
                    sh("sudo azcopy copy  '${storage_account}/${params.blob_source_container_name}/${params.datasets_folder}/${snapshot_id}${SAS_KEY}'  'data/input/DaVinciProjectOneDrive/' --recursive=true")
                    sh("sudo azcopy copy  '${storage_account}/${params.blob_source_container_name}/common/gtin_fpc_customer_mapping.xlsx${SAS_KEY}'  'data/input/DaVinciProjectOneDrive/${snapshot_id}/fpc_mapping/uk/gtin-fpc customer mapping.xlsx' ")
                    ...
                    sh("sudo azcopy copy  '${storage_account}/${target_folder}/future_predictions/${params.location}/${snapshot_id}/all${SAS_KEY}'  'data/lake/ml_results/' --recursive=true")
                    // Download DA results.
                    ...
                    sh("sudo azcopy copy 'data/lake/da_results/${snapshot_id}/${params.location}/'   '${storage_account}/${target_folder}/da_results/${snapshot_id}${SAS_KEY}'  --recursive=true")
    ...
    post {
        ...
        success {
            ...
            withCredentials([usernamePassword(credentialsId: "${api_token_shared_id_w_token}", usernameVariable: 'API_USER', passwordVariable: 'API_TOKEN')]) {
                script {
                    if (envir == 'PROD') {
                        echo "Checkpoint job URL params: ${checkpoint_job_url_params}"
                        sh("curl --insecure -X POST \"${host_prod}/job/UK-Checkpoint/buildWithParameters?${checkpoint_job_url_params}\" --user ${API_USER}:${API_TOKEN}")
                    }
                }
            }
        }
