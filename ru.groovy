                        ...
                        script {
                            if (StageSucceed) {
                                ...
                                withCredentials([usernamePassword(credentialsId: "${api_token_shared_id_w_token}", usernameVariable: 'API_USER', passwordVariable: 'API_TOKEN')]) {
                                    sh("curl --insecure -X POST \"${host_prod}/job/Fallback-RU/buildWithParameters?${fallback_job_url_params}\" --user ${API_USER}:${API_TOKEN}")
                                }
                            }
                        }
                    }
            ...
            withCredentials([usernamePassword(credentialsId: "${api_token_shared_id_w_token}", usernameVariable: 'API_USER', passwordVariable: 'API_TOKEN')]) {
                script {
                    if (envir == 'PROD') {
                        ...
                        sh("curl --insecure -X POST \"${host_prod}/job/RU-Checkpoint/buildWithParameters?${checkpoint_job_url_params}\" --user ${API_USER}:${API_TOKEN}")
                    }
                }
            }
        }
