...
/*
	parameters {
        ...
        string(name: 'ml_step_args', defaultValue: '-p checkpoint -x complex', description: 'Parameter for ML step.')
        ...
        stage('ML') {
            steps {
                ...
                withCredentials([string(credentialsId: "${blob_shared}", variable: 'BLOB_KEY'), string(credentialsId: "${sas_blob_shared}", variable: 'SAS_KEY')]) {
                    sh("sudo azcopy copy '${storage_account}/${target_folder}/snapshots/${params.location}/${params.etl_version}${SAS_KEY}' 'data/lake/snapshots/${params.location}' --recursive=true")
                    ...
                    sh("sudo azcopy copy 'data/experiments/${run_description}/all/all/predictions/*' '${storage_account}/${target_folder}/checkpoints/${params.location}/${params.snapshot_id}${SAS_KEY}' --recursive=true")
                    sh("sudo azcopy copy 'data/experiments/${run_description}/*' '${storage_account}/${target_folder}/checkpoints/${params.location}/${params.snapshot_id}${SAS_KEY}' --recursive=true")
                }
            }
        }
    }
